...
const CONCURRENT_CAPS = [0, 1, 2, 2, 3, 3]; // by wave
...
export class EnemyAttackSystem {
  ...
  update(dt, wave) {
    ...
    const maxConcurrent = CONCURRENT_CAPS[Math.min(wave + 1, CONCURRENT_CAPS.length - 1)] ?? 2;
  ...
  _countActiveDivers() {
    ...
    let n = 0;
    for (const e of enemies) if (e.active && e.state === 'DIVING') n++;
    return n;
  }
